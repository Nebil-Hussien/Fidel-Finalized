...
const Login = () => {
  ...
  const login = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(requests.login, loginData);
      if (!data.success) {
  ...
  return (
    <div className="register">
      ...
      <Form onSubmit={login}>
        <h1 className="heading-primary--main">{t("login_heading_primary_loginAdminPage")}</h1>
        <Form.Group>
        ...
        <Button
          disabled={isEmpty(userName, password) || isLoading}
          loading={isLoading}
          type="submit"
        >
          Login
